...
"""Script dat de files met de aangewezen map met xml_1 2 3 files uit folder
/Volumes/Afdeling prepress/SWITCH/PREPRESS OUTPUT/DISTRIFILL OUTPUT'
na bewerking door switch PITSTOP SERVER verplaatst naar hun itemnummer,
en naar de aangewezen ordermap om makkelijk naar Labelhub te verplaatsen.
"""
...
from Distrifill.distrifill_itemnummer import customernum, Mnumber, item_number, distrifill_rename, tuple_destinies
...
def main():
    ...
    if not fname:
        ...
        customernumbers = [(customernum(padxml[1]), Mnumber(padxml[1]), item_number(padxml[1]), padxml[1]) for padxml in
                           vila_1_xmls_are]

        customernum_dict = {f'{distrifill_rename(Mnumber(padxml[1]))}': (
            distrifill_rename(Mnumber(padxml[1])), item_number(padxml[1]), padxml[1]) for padxml in vila_1_xmls_are}
        #
        ...
        # and one that places them their esko itemnumber.
        #
